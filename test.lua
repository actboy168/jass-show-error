...
local function guiscript(script, lines)
    --TODO
...
local function guilabel(text)
    ...
    label:setstyle { Width=400, Left=20 }
...
title:setstyle { Height = 100, FlexDirection = 'row', JustifyContent = 'space-between' }
title:setmousedowncanmovewindow(true)
local label = guilabel('这是一个错误')
label:setmousedowncanmovewindow(true)
title:addchildview(label)
title:addchildview(guiclose(win))
view:addchildview(title)
view:addchildview(guiscript(readfile 'blizzard.j', '1000'))
